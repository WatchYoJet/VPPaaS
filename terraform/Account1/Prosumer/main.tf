...
resource "aws_instance" "prosumer" {
  ...
  tags = { Name = "vppas-prosumer" }
}

resource "aws_security_group" "instance" {
  name = "terraform-prosumer-account1-2026"
  ingress {
    from_port        = 0
    to_port          = 0
    protocol         = "-1"
...
output "public_dns" {
  value       = aws_instance.prosumer.public_dns
  description = "Prosumer service endpoint (port 8080)"
}
