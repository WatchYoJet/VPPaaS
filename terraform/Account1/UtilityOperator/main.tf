...
resource "aws_instance" "utilityoperator" {
  ...
  tags = { Name = "vppas-utilityoperator" }
}

resource "aws_security_group" "instance" {
  name = "terraform-utilityoperator-account1-2026"
  ingress {
    from_port        = 0
    to_port          = 0
    protocol         = "-1"
...
output "public_dns" {
  value       = aws_instance.utilityoperator.public_dns
  description = "UtilityOperator service endpoint (port 8080)"
}
