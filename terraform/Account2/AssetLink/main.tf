...
resource "aws_instance" "assetlink" {
  ...
  tags = { Name = "vppas-assetlink" }
}

resource "aws_security_group" "instance" {
  name = "terraform-assetlink-account2-2026"
  ingress {
    from_port        = 0
    to_port          = 0
    protocol         = "-1"
...
output "public_dns" {
  value       = aws_instance.assetlink.public_dns
  description = "AssetLink service endpoint (port 8080)"
}
