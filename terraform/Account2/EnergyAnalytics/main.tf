...
resource "aws_instance" "energyanalytics" {
  ...
  tags = { Name = "vppas-energyanalytics" }
}

resource "aws_security_group" "instance" {
  name = "terraform-energyanalytics-account2-2026"
  ingress {
    from_port        = 0
    to_port          = 0
    protocol         = "-1"
...
output "public_dns" {
  value       = aws_instance.energyanalytics.public_dns
  description = "EnergyAnalytics service endpoint (port 8080)"
}
