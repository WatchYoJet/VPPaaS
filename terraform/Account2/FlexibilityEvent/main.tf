...
resource "aws_instance" "flexibilityevent" {
  ...
  tags = { Name = "vppas-flexibilityevent" }
}

resource "aws_security_group" "instance" {
  name = "terraform-flexibilityevent-account2-2026"
  ingress {
    from_port        = 0
    to_port          = 0
    protocol         = "-1"
...
output "public_dns" {
  value       = aws_instance.flexibilityevent.public_dns
  description = "FlexibilityEvent service endpoint (port 8080)"
}
