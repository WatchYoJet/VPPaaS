...
variable "utilityoperator_url" { type = string }
variable "assetlink_url" { type = string }

resource "aws_instance" "gridbalancing" {
  ...
  user_data = base64encode(templatefile("startup.sh", {
    ...
    utilityoperator_url = var.utilityoperator_url
    assetlink_url       = var.assetlink_url
  }))
  user_data_replace_on_change = true

  tags = { Name = "vppas-gridbalancing" }
}

resource "aws_security_group" "instance" {
  name = "terraform-gridbalancing-account2-2026"
  ingress {
    from_port        = 0
    to_port          = 0
    protocol         = "-1"
...
output "public_dns" {
  value       = aws_instance.gridbalancing.public_dns
  description = "GridBalancing service endpoint (port 8080)"
}
