...
variable "utilityoperator_url" {
  type = string
}
variable "assetlink_url" {
  type = string
}
...
resource "aws_instance" "exampleDeployQuarkus" {
  ...
  user_data = base64encode(templatefile("MicroservicesCreation.sh", {
  ...
  tags = {
    Name = "terraform-quarkus-group-c"
  }
}

resource "aws_security_group" "instance" {
  name = "terraform-quarkus-group-c-2026"
  ingress {
    from_port        = 0
    to_port          = 0
    protocol         = "-1"
